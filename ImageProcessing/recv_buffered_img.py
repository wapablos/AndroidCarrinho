import socket
import sys
import cv2 as cv
import struct

ip = '192.168.43.1'
addr = (ip,9191)
addr2 =(ip,9192)

placa_cascade = cv.CascadeClassifier('cascades/placa_pare.xml')
semaforo_cascade = cv.CascadeClassifier('cascades/semaforo.xml')  # cascade detecta mais pela intensidade

blueColor=(255,0,0)
redColor=(0,0,255)
greenColor=(0,255,0)

# Detect vai ser substituida pode NN
def detect(img,myCascade,color,commSock):
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    object = myCascade.detectMultiScale(gray, 1.3, 3)
    if  len(object) == 0 : print("Empty")
    else :
        commSock.send('D'.encode())
        print('Received from server: ' + str(commSock.recv(1024)))
    for (x, y, w, h) in object: cv.rectangle(img, (x, y), (x + w, y + h), color, 2)


try:
    sock1 = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    sock2 = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
    print('Socket Initialized')
except:
    print('Failed to Init Socket')
    sys.exit(0)

try:
    sock1.connect(addr)
    sock2.connect(addr2)
    print('Socket Connected')
except:
    print('Connection Failed')
    sys.exit(0)

bufsize = 65536 # Buffer de 64KB

while True:
    with open('shot.jpg','wb') as f:
        data = sock1.recv(1)
        data = sock1.recv(bufsize)
        imgSize = struct.unpack('>i', data[0:4])
        if (data[-4:] == b'FEND'):
            f.write(data[9:])
        else:
            data+= sock1.recv(abs(imgSize[0]-bufsize))
            f.write(data[9:])

        try:
            img = cv.imread('shot.jpg')
            detect(img, semaforo_cascade, blueColor, sock2)
            cv.imshow("img", img)
            if cv.waitKey(25) == 27: break
        except: print('not image')
    f.close()

sock1.close()
sock2.close()